...
class SurfaceCBFQPGuidanceHelperTests(unittest.TestCase):
    def test_late_stage_guided_config_defaults_match_requested_tail_guidance(self):
        ...
        self.assertEqual(config.guidance_timesteps, (10, 5, 1))
        self.assertEqual(config.qp_candidates, 2)
        self.assertEqual(config.qp_inner_scp_rounds, 1)
        self.assertEqual(config.coarse_check_steps, 16)
